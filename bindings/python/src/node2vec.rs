...
impl SkipGram {
    ...
    ///     The graph to embed.
    fn fit_transform(&self, graph: &Graph) -> PyResult<Vec<Py<PyAny>>> {
        println!("Hellow from Node2VecBinding--Skipgtram");
        self.inner.fit_transform(graph)
    }
}
...
impl DreamWalk {
    #[new]
    #[args(py_kwargs = "**")]
    /// Return a new instance of the DreamWalk model.
    ///
    /// Parameters
    /// ------------------------
    /// embedding_size: Optional[int] = 100
    ...
    ///     random_state to use to reproduce the walks.
    /// iterations: int = 1
    ///     Number of cycles on the graphs to execute. Actually this is the number of complete
    ///     random walks (one RW per node) to perform per epoch (iterations=10 does 10x#nodes walks)
    /// max_neighbours: Optional[int] = 100
    ///     Maximum number of randomly sampled neighbours to consider.
    ///     If this parameter is used, the walks becomes probabilistic in nature
    ///     and becomes an approximation of an exact walk.
    ...
    /// verbose: bool = True
    ///     Whether to show the loading bar.
    pub fn new(py_kwargs: Option<&PyDict>) -> PyResult<DreamWalk> {
        Ok(Self {
            inner: Node2VecBinding::new(Node2VecModels::DreamWalkNodeAware, py_kwargs)?,
        })
    }
}
...
impl DreamWalk {
    ...
    ///     The graph to embed.
    fn fit_transform(&self, graph: &Graph) -> PyResult<Vec<Py<PyAny>>> {
        println!("Hellow from Dreamwalk-Node2VecBinding");
        self.inner.fit_transform(graph)
    }
}
